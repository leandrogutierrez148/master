...
select c.category_id, sum(oi.quantity) 
from orders o 
inner join order_items oi on oi.order_id = o.order_id 
inner join products p on p.product_id = oi.product_id 
...
inner join order_items oi on oi.order_id = o.order_id 
inner join products p on p.product_id = oi.product_id 
inner join categories on c.category_id = p.category_id 
where extract(year from o.order_date) = '2017' 
and o.status = 'Shipped' 
group by c.category_id 
;
...
inner join order_items oi on oi.order_id = o.order_id 
inner join products p on p.product_id = oi.product_id 
inner join product_categories on c.category_id = p.category_id 
where extract(year from o.order_date) = '2017' 
and o.status = 'Shipped' 
group by c.category_id 
;
...
select c.category_id, sum(oi.quantity) venta 
from orders o 
inner join order_items oi on oi.order_id = o.order_id 
inner join products p on p.product_id = oi.product_id 
inner join product_categories c on c.category_id = p.category_id 
...
select c.customer_id, sum(oi.quantity) venta 
from orders o 
inner join order_items oi on oi.order_id = o.order_id 
inner join products p on p.product_id = oi.product_id 
inner join customers c on c.customer_id = o.custormer_id 
...
select 	extract(year from o.order_date) año, 
...
select p.product_id, avg(list_price), list_price 
from products p;
...
with media as( 
    select avg(list_price) media 
    from products 
)  
select 	p.product_id,  
    	list_price 
from products p 
where list_price > (select media from media)
...
select 	p.product_id,  
    	list_price, (select media from media) 
from products p 
where list_price > (select media from media)
...
and o.status = 'Shipped' 
and venta > 50000 
group by e.employee_id, complete_name 
order by venta desc 
fetch first 1 rows only;
...
select count(customer_id) 
from customers  
where customer_id in (select customer_id from contacts);

select customer_id from contacts;

select  m.employee_id 
    	(m.first_name || m.last_name) as complete_name, 
        sum(oi.quantity * oi.unit_price) as venta 
from orders o 
inner join order_items oi on oi.order_id = o.order_id 
inner join employees e on e.employee_id = o.salesman_id 
